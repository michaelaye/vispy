...
class Canvas(app.Canvas):
    ...
    def __init__(self, fname, tindex, psize_scale=1):
        ...
        print("Max/min: {}, {}".format(inputdata[:, 2].max(),
                                       inputdata[:, 2].min()))
        # rescale
        # factor = inputdata.max()/2.3  ## 2.3 was max of original data
        # factor = inputdata.max()/20.0  ## 2.3 was max of original data
        r = 252000.0
        # inputdata /= factor
        inputdata /= r
        ...
        # Create vertices
        n = inputdata.shape[0]+301
        data = np.zeros(n, [('a_position', np.float32, 3),
                            ('a_bg_color', np.float32, 4),
                            ('a_fg_color', np.float32, 4),
                            ('a_size', np.float32, 1)])
        data['a_position'][:-301] = inputdata
        data['a_bg_color'][:-301] = np.random.uniform(0.75, 1.0, (n-301, 4))
        ...
        data['a_size'][-1] = 252*ps
        data['a_bg_color'][-301:] = np.array([0.5,0.6,0.7,0.8])
